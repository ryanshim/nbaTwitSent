...
router.get('/tweetscore', function(req, res) {
    let sql = "SELECT team, AVG(score) as average, COUNT(score) as count FROM tweets GROUP BY team";
    var params = []
...
router.get('/:team/tweetscore', function(req, res) {
    let sql = "SELECT team, AVG(score) as average, COUNT(score) as count FROM tweets WHERE team = ?";
    var params = [req.params.team.toUpperCase()];
    tweets_query(sql, params, res);
});
